...
## Hosted editor boundary

The hosted editor at https://app.ttyinv.com keeps invoice Markdown and uploaded fonts in browser storage. Its server stores only GitHub's durable numeric user ID and current login, hashed session tokens, and payment identifiers, amounts, currencies, states, and timestamps. GitHub access tokens are discarded after login, and invoice content is never attached to authentication or payment requests. Explicit REST and MCP payloads are transient and never logged or stored. See the [hosted service documentation](https://app.ttyinv.com/docs) for the current hosted-service policy.
