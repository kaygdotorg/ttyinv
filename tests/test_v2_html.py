...
def test_enhancer_adds_accessible_semantics_and_frame(tmp_path: Path) -> None:
    ...
    assert len(soup.select(".page-frame")) == 1
    assert [corner.get_text(strip=True) for corner in soup.select(".frame-corner")] == ["+"] * 4
...
def test_unsafe_and_escaping_links_are_deactivated(tmp_path: Path) -> None:
    source = tmp_path / "source" / "invoice.md"
    source.parent.mkdir()
    html = HTML.replace(
        "</body>",
        ...
        '<a href="../private.pdf">Escape</a></body>',
    )
    rendered = enhance_html(
        html,
        EnhanceOptions(
            source,
            tmp_path / "invoice.html",
            resolve_palette("light"),
        ),
    )
    soup = BeautifulSoup(rendered, "html.parser")
    blocked = soup.select("[data-ttyinv-blocked-link]")
    assert [link.get("href") for link in blocked] == [None, None, None]
    assert [link["data-ttyinv-blocked-link"] for link in blocked] == [
        "data",
        "javascript",
        "local-path",
    ]
