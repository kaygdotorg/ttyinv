...
def test_consecutive_sections_get_extra_breathing_room_without_moving_forced_breaks(tmp_path: Path) -> None:
    html = _render(tmp_path)

    # The shared sibling rule spaces generated sections consistently in both
    # browser previews and print output. Explicit page-break sections are
    # excluded so their continuation-page top inset remains calibrated.
    assert "--section-gap-extra: 2mm" in html
    assert '.document-section + .document-section:not([data-page-break-before="true"]) { margin-top: calc(var(--section-gap) + var(--section-gap-extra)); }' in html
