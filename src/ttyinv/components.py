...
def render_financial_section(section: CalculatedFinancialSection, currency: str, locale: str, *, show_section_total: bool, grand_total: str | None, section_id_value: str) -> tuple[str, list[float]]:
    ...
    header_classes = [align_class(section.align[i] if i < len(section.align) else None, i == section.payable_amount_column) for i in range(len(section.headers))]
    ...
    subtotal = render_summary(widths=widths, amount_column=section.payable_amount_column, label=section.title, amount=display_money(section.total, currency, locale), css_class="section-total") if show_section_total else ""
...
<div class="section-tail">
  ...
  {total_due}
    ...
    return render_section(title=section.title, section_id_value=section_id_value, body=body, css_class="document-section financial-section", attributes=f'data-column-widths="{",".join(f"{width:g}" for width in widths)}"'), widths


def render_prose_section(section: CalculatedSection, section_id_value: str) -> str:
    return render_section(title=section.title, section_id_value=section_id_value, body=section.html, css_class="document-section prose-section")
