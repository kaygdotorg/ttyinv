...
def _is_external_link(href: str) -> bool:
    ...
    return parsed.scheme in {"http", "https", "mailto", "data", "tel"} or href.startswith("#")
...
<article class="invoice-sheet" aria-labelledby="invoice-title">
  ...
  <div class="footer-stack">{payment_html}{signature_html}</div>
</article>
</body></html>'''
    result = _rewrite_document(document, invoice, options)
    result.warnings = sorted(set(result.warnings + font_assets.warnings + appearance_warnings))
    return result
