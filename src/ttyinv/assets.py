...
def embed_asset_path(asset_path: Path) -> str:
    mime = _SUPPORTED_MIME.get(asset_path.suffix.lower())
    if not mime:
        raise TtyinvError(f"Cannot embed asset {asset_path}: unsupported file type.")
    try:
        if asset_path.stat().st_size > _MAX_ASSET_BYTES:
            raise TtyinvError(f"Cannot embed asset {asset_path}: file exceeds the 20 MiB limit.")
